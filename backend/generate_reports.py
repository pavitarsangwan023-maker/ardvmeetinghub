...
<body>
    <div class="cover">
        ...
        <p style="font-size: 1.2em;"><strong>Developed By:</strong> Pavitar Sangwan</p>
...
<body>
    ...
    <p><strong>Student Name:</strong> Pavitar Sangwan</p>
...
async def run():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.goto("http://localhost:5173/register", timeout=15000, wait_until="networkidle")
            await page.wait_for_timeout(2000)
            await page.screenshot(path="screenshot_register.png")
            print("Captured screenshot_register.png")
            
            # Register a dummy user to access dashboard
            random_email = f"admin_{uuid.uuid4().hex[:6]}@example.com"
            await page.fill("input[placeholder='Full name']", "Presentation User")
            ...
            await page.wait_for_timeout(3000) # Wait for all elements to load
            await page.screenshot(path="screenshot_dashboard.png")
            print("Captured screenshot_dashboard.png")
            
            # Navigate to meeting room (fake room)
            print("Navigating to Meeting Room...")
            await page.click("button:has-text('New Meeting')")
            # This triggers a new meeting and redirects to /meeting/...
            await page.wait_for_timeout(4000) # wait for meeting UI to load
            # Deny camera/mic if prompted (Playwright automatically denies by default if not granted)
            await page.screenshot(path="screenshot_meeting.png")
            print("Captured screenshot_meeting.png")
            
        except Exception as e:
            print("Error capturing some screenshots:", e)

        await browser.close()

    print("Converting images to base64...")
    # Convert images to base64 so they embed perfectly in PDF regardless of path
    def get_b64(path):
    ...
    async with async_playwright() as p:
        ...
        major_content = MAJOR_HTML.format(
        ...
        await page.pdf(path="../Major_Project_Report_Ardvmeetinghub.pdf", format="A4", margin={"top": "20px", "bottom": "20px", "left": "20px", "right": "20px"})
        print("Major Report Saved.")
        
        minor_content = MINOR_HTML.format(
            dashboard_img=dashboard_b64,
            meeting_img=meeting_b64
        )
        await page.set_content(minor_content, wait_until="networkidle")
        await page.pdf(path="../Minor_Project_Report_Ardvmeetinghub.pdf", format="A4", margin={"top": "20px", "bottom": "20px", "left": "20px", "right": "20px"})
        print("Minor Report Saved.")
